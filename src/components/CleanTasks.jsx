...
export default function CleanTasks({task, dispatch, isKawaii}) {
    return (
        <footer className="border-t border-zinc-200 mt-9 px-10 py-7 flex flex-col gap-4 md:flex-row md:justify-between">
            ...
            {task.length > 0 && (
                <button
                className={`text-sm md:text-base text-center text-red-500 font-semibold px-7 py-2 border border-zinc-200 rounded-md flex items-center gap-2 justify-center cursor-pointer hover:bg-red-50 hover:shadow transition-all ${isKawaii ? 'bg-pink-100' : ''}`}
