...
export default function AddTasks({ dispatch, state, isKawaii }) {
    ...
    return (
        <form className="bg-purple-100 w-11/12 p-2.5 rounded-md mx-auto my-8 flex flex-col gap-2.5 md:flex-row">
            <input
                type="text"
                ref={inputRef}
                placeholder={error ? 'Agrega un texto' : isKawaii ? '🐇 Qué tarea necesitas hacer?' : 'Qué tarea necesitas hacer?'}
                className={`bg-white py-2 px-3 min-w-4/5 ${error ? 'border-2 border-red-500 placeholder:text-red-400' : 'border-neutral-50'}`}
                value={task}
                onChange={handleTask} />
            <button
                type="submit"
                className={`w-full text-white md:w-1/5 rounded-md py-2 px-3 cursor-pointer hover:shadow transition-all ${state.activeTask ? 'bg-yellow-600 hover:bg-yellow-500' : isKawaii ? 'bg-pink-300 hover:bg-pink-400' : 'bg-indigo-700 hover:bg-indigo-600' }`}
